...
int main(int argc, const char *argv[]){
    ...
    // (3.2) Training for SVM
    OC_SVM svm(K, params, vm["verbose"].as<bool>());
    svm.train(train_data, vm["nd"].as<size_t>(), vm["nu"].as<double>(), vm["lr"].as<double>());
    ...
    // (4.3) Test for SVM
    svm.test(test_normal_data, test_anomaly_data);
    std::cout << "///////////////////////// Test /////////////////////////" << std::endl;
    std::cout << "accuracy-all: " << svm.accuracy << " (" << svm.correct_n + svm.correct_a << "/" << test_normal_data.size() + test_anomaly_data.size() << ")" << std::endl;
    std::cout << "accuracy-normal: " << svm.accuracy_n << " (" << svm.correct_n << "/" << test_normal_data.size() << ")" << std::endl;
    std::cout << "accuracy-anomaly: " << svm.accuracy_a << " (" << svm.correct_a << "/" << test_anomaly_data.size() << ")" << std::endl;
    std::cout << "AUROC: " << svm.auroc << std::endl;
    std::cout << "////////////////////////////////////////////////////////" << std::endl;
